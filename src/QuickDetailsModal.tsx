...
export const QuickDetailsModal = ({ remedy, onClose }: { remedy: MateriaRemedy, onClose: () => void }) => {
  ...
  const bilingualData = [
    ...
    {
      enLabel: 'Guiding Symptoms',
      bnLabel: 'প্রধান লক্ষণ',
      enData: remedy.guidingSymptoms || [],
      bnData: remedy.banglaGuidingSymptoms || [],
    },
    {
      enLabel: 'Clinical Indications',
      bnLabel: 'ক্লিনিকাল নির্দেশ',
      enData: remedy.clinicalIndications || [],
      bnData: remedy.banglaClinicalIndications || [],
    },
    {
                ...
                <div className="md:col-span-4 bg-surface-container-low rounded-xl p-6 flex flex-col">
                  ...
                  <div className="space-y-6 flex-1">
                    ...
                    {remedy.relationships && (
                      <div className="p-4 bg-tertiary-container/10 rounded-xl border border-tertiary/20">
                        <p className="text-[10px] font-bold text-tertiary uppercase tracking-tighter mb-2">
                          {language === 'en' ? 'Relationships' : 'সম্বন্ধ'}
                        </p>
                        <div className="space-y-2 text-[10px]">
                          {remedy.relationships.complementary && (
                            <p><span className="font-bold">Comp:</span> {language === 'en' ? remedy.relationships.complementary : remedy.relationships.banglaComplementary}</p>
                          )}
                          {remedy.relationships.antidote && (
                            <p><span className="font-bold">Antidote:</span> {language === 'en' ? remedy.relationships.antidote : remedy.relationships.banglaAntidote}</p>
                          )}
                          {remedy.relationships.inimical && (
                            <p><span className="font-bold text-error">Inimical:</span> {language === 'en' ? remedy.relationships.inimical : remedy.relationships.banglaInimical}</p>
                          )}
                        </div>
                      </div>
                    )}
