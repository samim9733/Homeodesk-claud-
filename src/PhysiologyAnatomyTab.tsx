...
import { Microscope,  Brain, Activity, Zap, Utensils, Bone, ArrowRight, Edit} from 'lucide-react';
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700">
      ...
      <section className="bg-white/80 backdrop-blur-xl p-8 rounded-3xl border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <span className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] mb-2 block">{t('physiology.sectionOverview')}</span>
        ...
        <div className="flex gap-4">
          <div className="text-right">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t('patients.sortBy')}</p>
            <div className="flex gap-2">
              <span className="px-3 py-1 bg-emerald-50 text-emerald-600 text-[10px] font-black rounded-full border border-emerald-100 uppercase tracking-wider">{t('physiology.clinical')}</span>
              <span className="px-3 py-1 bg-blue-50 text-blue-600 text-[10px] font-black rounded-full border border-blue-100 uppercase tracking-wider">{t('physiology.homeopathicLink')}</span>
            </div>
          </div>
        </div>
      </section>
          ...
          return (
            ...
            >
              ...
              <div className="mt-6 flex items-center justify-between">
                <div className="flex gap-2">
                  <span className="text-[10px] font-black uppercase tracking-wider text-emerald-600">{system.details.length} {t('physiology.rubrics')}</span>
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-300">•</span>
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">{t('physiology.criticalSystem')}</span>
                </div>
      ...
      {selectedSystem && (
        <section className="flex flex-col lg:flex-row gap-12 pt-8 border-t border-slate-100">
          <div className="flex-1 space-y-8">
            <div>
              <span className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] mb-2 block">{t('physiology.deepDive')}</span>
              <h3 className="text-5xl font-black text-slate-900 leading-tight tracking-tighter">
                {t('physiology.anatomyOf')} {selectedSystem.name.replace(' System', '')}<br/>
            ...
            <div className="space-y-4 pt-4">
              <h5 className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t('physiology.relevantRubrics')}</h5>
                ...
                <div className="bg-white border-l-4 border-emerald-500 p-5 rounded-2xl shadow-sm hover:shadow-md transition-all cursor-pointer group">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1">{t('library.pathologyMenu')}</p>
                    </div>
                    <span className="bg-emerald-50 text-emerald-600 px-2 py-1 rounded-lg text-[10px] font-black uppercase">42 {t('common.remedies')}</span>
                  </div>
                </div>
                <div className="bg-white border-l-4 border-blue-500 p-5 rounded-2xl shadow-sm hover:shadow-md transition-all cursor-pointer group">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1">{t('physiology.connectiveTissue')}</p>
                    </div>
                    <span className="bg-blue-50 text-blue-600 px-2 py-1 rounded-lg text-[10px] font-black uppercase">18 {t('common.remedies')}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="sticky top-8 bg-slate-50 rounded-[2.5rem] p-3 border border-slate-100">
              <div className="aspect-[3/4] rounded-[2rem] overflow-hidden relative group">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-8">
                  <p className="text-white/60 text-[10px] font-black uppercase tracking-widest mb-1">{t('physiology.anatomyFigure')}</p>
                  <p className="text-white font-black text-2xl tracking-tight">{selectedSystem.name} {t('physiology.anatomySectioning')}</p>
                  <button 
                  ...
                  >
                    <Edit size={14} /> {t('physiology.updateDiagram')}
                  </button>
                </div>
              </div>
              <div className="p-6 space-y-6">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('physiology.anatomyTag')}</span>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-xs font-bold text-slate-700">{t('physiology.primaryStructure')}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]"></div>
                    <p className="text-xs font-bold text-slate-700">{t('physiology.connectiveTissue')}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 rounded-full bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]"></div>
                    <p className="text-xs font-bold text-slate-700">{t('physiology.neuralInterface')}</p>
                  </div>
                </div>
              </div>
            </div>
