...
export const CHAPTER_INDEX: ChapterIndex[] = [
  ...
  { n: "Dreams", p: "1234-1258", i: "💭", t: "স্বপ্ন", c: "#4338ca" },
...
export const CHAPTER_CATEGORIES = [
  { name: 'Mind & Nervous System', t: 'মন ও স্নায়ুতন্ত্র', chapters: ['Mind', 'Vertigo', 'Sleep', 'Dreams'] },
...
export function cleanRubricName(name: string): string {
  if (!name) return '';
  const parts = name.split(',');
  let startIndex = 0;
  while (startIndex < parts.length) {
    const part = parts[startIndex].trim();
    if (part.endsWith('.') && part.length <= 15) { // Remedy abbreviation is usually short and ends with a dot
      startIndex++;
    } else {
      break;
    }
