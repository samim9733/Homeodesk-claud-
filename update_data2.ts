    ...
    if (titleMatch) {
       ...
       current = {
        ...
        if (line.startsWith('লক্ষণ:')) {
            ...
            // Note: Data is comma-separated now instead of semicolon for medicines!
...
const originalData = JSON.parse(content.substring(content.indexOf('['), content.lastIndexOf('];') + 1));
// Only keep first 30
const trimmedData = originalData.slice(0, 30);
const combinedData = [...trimmedData, ...results];

const exportIndex = content.indexOf('export const DIAGNOSES_DATA: ClinicalDiagnosis[] =');
if (exportIndex !== -1) {
    const jsonStr = JSON.stringify(combinedData, null, 2);
    const newContent = content.substring(0, exportIndex) + 'export const DIAGNOSES_DATA: ClinicalDiagnosis[] = ' + jsonStr + ';\n';
    fs.writeFileSync('src/practiceMedicineData.ts', newContent);
    console.log('Successfully rewrote array with ' + combinedData.length + ' records.');
} else {
    console.log('Export array start not found!');
}
